...
exports.index = function (req, res, next) {
  ...
  ep.once('topic', function (topic) {
    ...
    at_ctrl.linkUsers(topic.content, ep.done(function (content) {
      topic.content = Util.xss(Showdown.parse(content));
      ep.emit('@user');
    }));
  });

  getFullTopic(topic_id, ep.done(function (message, topic, tags, author, replies) {
    if (message) {
      ep.unbind();
    ...
    exports.getTopicsByQuery(query, options, ep.done('other_topics'));

    // get no reply topics
    var options2 = { limit: 5, sort: [ ['create_at', 'desc'] ] };
    exports.getTopicsByQuery({reply_count: 0}, options2, ep.done('no_reply_topics'));
  }));
};
...
exports.create = function (req, res, next) {
  ...
  if (method === 'get') {
    tag_ctrl.getAllTags(function (err, tags) {
      if (err) {
        return next(err);
      }
    ...
    if (title === '') {
      ...
      topic.save(function (err) {
        ...
        topic_tags.forEach(function (tag) {
          ...
          tag_ctrl.getTagById(tag, proxy.done(function (tag) {
            tag.topic_count += 1;
            tag.save();
          }));
        });
        user_ctrl.getUserById(req.session.user._id, proxy.done(function (user) {
          user.score += 5;
          user.topic_count += 1;
          user.save();
        ...
        //发送at消息
        at_ctrl.sendMessageToMentionUsers(content, topic._id, req.session.user._id);
      });
    }
  }
};
...
exports.edit = function (req, res, next) {
  ...
  if (method === 'get') {
    ...
    exports.getTopicById(topic_id, function (err, topic, tags) {
      if (!topic) {
        res.render('notify/notify', {error: '此话题不存在或已被删除。'});
        return;
      }
      if (topic.author_id === req.session.user._id || req.session.user.is_admin) {
        tag_ctrl.getAllTags(function (err, all_tags) {
          if (err) {
            return next(err);
          }
  ...
  if (method === 'post') {
    ...
    exports.getTopicById(topic_id, function (err, topic) {
      if (!topic) {
        res.render('notify/notify', {error: '此话题不存在或已被删除。'});
        return;
      }
              ...
              if (docs.length === 0) {
                ...
                docs.forEach(function (doc) {
                  doc.remove(proxy.done(function () {
                    tag_ctrl.getTagById(doc.tag_id, proxy.done(function (tag) {
...
exports.getTopicById = function (id, cb) {
  var proxy = new EventProxy();
  ...
  Topic.findOne({_id: id}, proxy.done(function (topic) {
    ...
    TopicTag.find({topic_id: topic._id}, proxy.done(function (topic_tags) {
      ...
      tag_ctrl.getTagsByIds(tags_id, proxy.done('tags'));
    }));

    user_ctrl.getUserById(topic.author_id, proxy.done('author'));

    if (topic.last_reply) {
      reply_ctrl.getReplyById(topic.last_reply, proxy.done(function (last_reply) {
...
exports.getFullTopic = function (id, cb) {
  ...
  Topic.findOne({_id: id}, proxy.done(function (topic) {
    ...
    TopicTag.find({topic_id: topic._id}, proxy.done(function (topic_tags) {
      ...
      tag_ctrl.getTagsByIds(tags_ids, proxy.done('tags'));
    }));

    user_ctrl.getUserById(topic.author_id, proxy.done(function (author) {
      if (!author) {
        proxy.unbind();
    ...
    reply_ctrl.getRepliesByTopicId(topic._id, proxy.done('replies'));
  }));
}
